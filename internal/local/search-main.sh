...
[[ -z $(docker ps | grep ${CONTAINER_NAME}) ]] \
	&& docker run -d --rm --name ${CONTAINER_NAME} -v ${DIR_PATH}:${WORK_DIR}:ro ${DOCKER_IMAGE}

if [[ -z ${TEX_FILE_PATH} ]]; then

	result=$(docker exec ${CONTAINER_NAME} /bin/ash -c "cd ${WORK_DIR} && python3 ${WORK_DIR}/${LOCAL_PATH} ${CONTAINER_USER_DIR} ${WORK_DIR}")

else

	result=$(docker exec ${CONTAINER_NAME} /bin/ash -c "cd ${WORK_DIR} && python3 ${WORK_DIR}/${LOCAL_PATH} ${CONTAINER_USER_DIR} ${WORK_DIR} ${TEX_FILE_PATH}")
